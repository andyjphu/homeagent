 ...
 * 1. Checks current pipeline stage
 * 2. Runs the next batch of work (enrich a property, score properties)
 * 3. Advances to next stage when done
 *
 * Pipeline stages:
 *   searching → enrichment (per property via API providers) → scoring → complete
 */
export async function POST(request: Request) {
  ...
  try {
    switch (stage) {
      case "enrichment":
        return NextResponse.json(
          await handleEnrichmentStage(taskId, task.buyer_id, outputData)
        );
      case "scoring":
        return NextResponse.json(
          await handleScoringStage(taskId, task.buyer_id, outputData)
        );
      default:
        return NextResponse.json({
          stage: stage ?? "pending",
...
/**
 * Enrich one property per process call using the API enrichment service.
 * Advances enrichment_index until all properties are enriched, then moves to scoring.
 */
async function handleEnrichmentStage(
  ...
  if (currentIndex >= propertyIds.length) {
    // All properties enriched — move to scoring
  ...
  if (lat && lng) {
    // Run enrichment via API providers (parallel, fast)
    try {
      const result = await enrichProperty(lat, lng, fullAddress);

      // Store enrichment data on property
      ...
      if (enrichUpdateErr) {
        if (enrichUpdateErr.message?.includes("does not exist")) {
          console.warn(`[process] enrichment_data column missing for ${propId} — run migration 00003`);
        } else {
...
async function startScoringStage(
  ...
  if (!buyerId || propertyIds.length === 0 || !isLLMAvailable("property_scoring")) {
    const reason = !isLLMAvailable("property_scoring")
      ? "AI not configured — skipping scoring"
      : "No properties to score";

    if (!isLLMAvailable("property_scoring") && propertyIds.length > 0) {
      await logEvent(taskId, "stage_scoring_failed", { error: reason });
    }
...
/**
 * Run LLM scoring for all properties against buyer intent.
 */
async function handleScoringStage(
  ...
  // Score all properties in parallel
